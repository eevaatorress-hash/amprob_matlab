...
% PRACTICA 7: Navegación autónoma utilizando el algoritmo A*
...
heu = zeros(size(costes));
nuevos_costes = zeros(size(costes));
mask = (costes ~= 0);
[columna,fila] = find(mask == 1);
dist = sqrt((nodos(fila,2)-nodos(columna,2)).^2 + (nodos(fila,3)-nodos(columna,3)).^2);

idx = sub2ind(size(nuevos_costes), fila, columna);
nuevos_costes(idx) = dist;

mask = (costes ~= -1);
[columna,fila]=find(mask == 1);
dist_heu = sqrt((nodos(fila,2)-nodos(columna,2)).^2 + (nodos(fila,3)-nodos(columna,3)).^2);
idx = sub2ind(size(heu), fila, columna);
heu(idx) = dist_heu;
...
function [coste,ruta] = aestrella(Grafo,Heuristica,inicio,destino)
    ...
    camino = [nodos,gn,fn,p];
    camino(inicio,2) = 0;
    camino(inicio,3) = 0;
    
    % Sacamos los valores estimados de cada nodo para llegar al destino
    hn = Heuristica(destino,:);
    ...
    while ~visitado(destino)
        ...
        nuevo_gn = camino(nodo_act,2) + fila;
        nuevo_fn = nuevo_gn + hn(nodo_act);
        mask = (fila > 0) & ~visitado & (camino(:,3) > nuevo_fn);
        camino(mask,2) = nuevo_gn(mask);
        camino(mask,3) = nuevo_fn(mask);
        camino(mask,4) = nodo_act;
        
        visitado(nodo_act) = true;
    
        % Elegimos el siguiente nodo no visitado con menor coste acumulado
        costes_temp = camino(:,3);
